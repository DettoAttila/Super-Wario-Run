import {player, animazione_idle, animazione_dashing, animazione_run} from './classes/player.js';
...
//GUI
const gem_counter_img = new Image();
gem_counter_img.src = "assets/num.png";
gem_counter_img.onload = drawGUI;

//Schermate
const title_img = new Image();
title_img.src = "assets/title.png";
//title_img.onload = drawTitle;

const gameover_img = new Image();
gameover_img.src = "assets/gameover.png";
//gameover_img.onload = drawGameOver;
...
let fg_tilemap = [[1073742003, 1073742004, 1073741987, 1073741988, 1073741971, 1073742003, 1073742004, 1073741987, 1073741988, 1073741971, 1073742003, 1073742004, 1073741987, 1073741988, 1073741971,
...
function drawStage(){
    ...
    if(frame_counter >= 10) { //delay
        frame_counter = 0;
        frame_index = (frame_index + 1) % 4; //4 sono i frame totali delle gemme
    }
        ...
        for(let livello of sezione.fg_tilemap){
            for (let i = inizio_row_visibile; i < fine_row_visibile; i++) {
                ...
                for (let j = inizio_col_visibile; j < fine_col_visibile; j++) {
                    ...
                    if(tile > 12 && tile < 15){
                        tile = 12 + frame_index + 1;
                    } else if(tile > 28 && tile < 31){
                        tile = 28 + frame_index + 1;
                    }
            ...
            for (let i = inizio_row_visibile; i < fine_row_visibile; i++) {
                ...
                for (let j = inizio_col_visibile; j < fine_col_visibile; j++) {
                    ...
                    if(collision_data[tile_id]){
                        ...
                        if (collisionInfo.type == "rect" || collisionInfo.type == "gem") {
                            tilebox = new SAT.Box(
...
function checkOnGround(tileCollisions){
    ...
    for(let tile of tileCollisions){
        ...
        if(SAT.testPolygonPolygon(player.collision_box.toPolygon(), tile.box.toPolygon ? tile.box.toPolygon() : tile.box, response)){
            if(response.overlapV.y > 0){ //player in piedi sul tile
                return true;
            }
        }
    }
    ...
    const testbox = new SAT.Box(new SAT.Vector(player.x - 5, player.y + 3), player.width, player.height - 10);

    for(let tile of tileCollisions){
        response.clear();

        if(SAT.testPolygonPolygon(testbox.toPolygon(), tile.box.toPolygon ? tile.box.toPolygon() : tile.box, response)){     
            if(response.overlapV.y > 0){ //player in piedi sul tile
                return true;
            }
        }
...
function risolviCollisione(tileCollisions){ //per riposizionare il personaggio
    ...
    for(let tile of tileCollisions){
        ...
        if(SAT.testPolygonPolygon(player.collision_box.toPolygon(), tile.box.toPolygon ? tile.box.toPolygon() : tile.box, response)){            
            ...
            if(tile.type == "gem" && (response.overlapV.y > 0 || response.overlapV.x > 0 || response.overlapV.y < 0)){
                tile.sezione.fg_tilemap[0][tile.index] = 0;
                fg_tilemap[0][tile] = 0;
                player.gemme += (tile.tile >= 12 && tile.tile <= 15)? 1 : 2;
                continue;
            }

            let blocco0 = [73,89]; //RIMOSSI 74, 76, 90 E 92 PER EVITARE BLOCCHI DISTRUTTI A METÀ
            let blocco1 = [75,91];

            //Se ci si trova su un tile solido
            if(response.overlapV.y > 1){ //player in piedi sul tile
                player.y -= response.overlapV.y; // + 3;
                player.salto = 0;
            } 
            
            if(response.overlapV.x > 1 || response.overlapV.x < 0){
                //se ti si para un blocco davanti
                if(blocco0.includes(tile.tile) || blocco1.includes(tile.tile)){
                    //se lo colpisci

                    if(player.stato == "dash"){
                        tile.sezione.fg_tilemap[1][tile.index] = 0;
                        tile.sezione.fg_tilemap[1][tile.index + 1] = 0; //così, se viene colpito il 73, si distrugge il 74 e così via

                        const offset_colonne = (tile.tile == 89 || tile.tile == 91) ? -tile.sezione.map_col : tile.sezione.map_col;

                        tile.sezione.fg_tilemap[1][tile.index + offset_colonne] = 0;
                        tile.sezione.fg_tilemap[1][tile.index + 1 + offset_colonne] = 0;
    ...
    //PER GESTIRE LE DISCESE
    const testbox = new SAT.Box(new SAT.Vector(player.x - 1, player.y), player.width, player.height - 2);
    
    for(let tile of tileCollisions){
        response.clear();

        if(tile.type == "poly" && SAT.testPolygonPolygon(testbox.toPolygon(), tile.box.toPolygon ? tile.box.toPolygon() : tile.box, response)){ 
            //console.log('discesa rilevata');
...
function update(time){
    ...
    if(player.stato == "dash"){
        ...
        player.rincorsa -= player.rallentamento;
    }
    
    //Per ragioni grafiche e fisiche
    ...
    if(player.x <= 10 && player.stato == "dash"){
        player.stato = "run";
        player.x = 10;
        player.rincorsa = 0;
    }
    ...
    //console.log(player.stato);

    if(player.y > canvas.height + tile_size || player.x < -player.width){ //GAME OVER
...
document.addEventListener("keydown", (e) => { //solo per prova, da sistemare meglio successivamente
    if(e.code == "Enter"){
        if(stato_gioco == "menu")
            start();
